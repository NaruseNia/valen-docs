...
If you've ever wanted `enum` to carry data, `match` to yell at you for missing a case, and `null` to simply *not exist*, you're in the right place.
...
| Exception | Java FFI boundary only |
...
Java's `null` becomes `Option` at the boundary. Java exceptions become `Result` via `safe { ... }`. The interop is intentionally boring — in the best way.
...
| Null safety | `Optional` / annotations | `?` nullable types | `Option` | **`Option<T>` only, no null** |
